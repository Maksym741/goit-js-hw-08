...
function onImageClick(event) {
  ...
  const imageEl = imageCardEl.querySelector(".gallery-image");

  const image = {
    original: imageEl.dataset.source,
    preview: imageEl.src,
    description: imageEl.alt,
  };

  const instance = basicLightbox.create(`
    <div class="modal">
      <li class="gallery-item">
        <a href="${image.original}" onclick="event.preventDefault()">
          <img
            src="${image.preview}"
            data-source="${image.original}"
            alt="${image.description}"
